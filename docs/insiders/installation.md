...
*griffe-autodocstringstyle Insiders* is a compatible drop-in replacement for *griffe-autodocstringstyle*,
...
*griffe-autodocstringstyle Insiders* can be installed with `pip` [using SSH][using ssh]:

```bash
pip install git+ssh://git@github.com/pawamoy-insiders/griffe-autodocstringstyle.git
```
...
```bash
pip install git+https://${GH_TOKEN}@github.com/pawamoy-insiders/griffe-autodocstringstyle.git
```
...
Of course, you can use *griffe-autodocstringstyle Insiders* directly using Git:

```
git clone git@github.com:pawamoy-insiders/griffe-autodocstringstyle
```

When cloning with Git, the package must be installed:

```
pip install -e griffe-autodocstringstyle
```

## Upgrading

When upgrading Insiders, you should always check the version of *griffe-autodocstringstyle*
