...
import griffe_autodocstringstyle
...
def _fixture_loader() -> griffe.GriffeLoader:
    ...
    loader.load("griffe_autodocstringstyle")
...
def _fixture_internal_api(loader: griffe.GriffeLoader) -> griffe.Module:
    return loader.modules_collection["griffe_autodocstringstyle._internal"]


@pytest.fixture(name="public_api", scope="module")
def _fixture_public_api(loader: griffe.GriffeLoader) -> griffe.Module:
    return loader.modules_collection["griffe_autodocstringstyle"]
...
def test_exposed_objects(modulelevel_internal_objects: list[griffe.Object | griffe.Alias]) -> None:
    """All public objects in the internal API are exposed under `griffe_autodocstringstyle`."""
    not_exposed = [
        obj.path
        for obj in modulelevel_internal_objects
        if obj.name not in griffe_autodocstringstyle.__all__ or not hasattr(griffe_autodocstringstyle, obj.name)
    ]
...
def test_single_locations(public_api: griffe.Module) -> None:
    ...
    multiple_locations = {}
    for obj_name in griffe_autodocstringstyle.__all__:
...
def test_inventory_matches_api(
    ...
    public_api_paths.add("griffe_autodocstringstyle")
    for item in inventory.values():
        if (
            item.domain == "py"
            and "(" not in item.name
            and (item.name == "griffe_autodocstringstyle" or item.name.startswith("griffe_autodocstringstyle."))
        ):
